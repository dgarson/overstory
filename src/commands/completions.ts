...
export const COMMANDS: readonly CommandDef[] = [
	...
	},
	{
		...
		subcommands: [
			{
				name: "start",
				desc: "Start the control daemon",
				flags: [{ name: "--json", desc: "JSON output" }],
			},
			{
				name: "stop",
				desc: "Stop the control daemon",
				flags: [{ name: "--json", desc: "JSON output" }],
			},
			{
				name: "status",
				desc: "Show control daemon status",
				flags: [{ name: "--json", desc: "JSON output" }],
			},
		],
	},
...
export function generateBash(): string {
	const lines: string[] = [
		...
		"",
		"  local commands='init sling prime status dashboard inspect merge nudge clean doctor log logs watch trace errors feed replay costs metrics spec coordinator control supervisor hooks monitor mail group worktree run codex'",
		"",
